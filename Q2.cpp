...

{
  ...
  if ( n == 1 )                // Base case of the recursion 
   ...
   else if ( n  > 1 )                // This if statement only enters when the else if ( savior) is not entered , this if prints the bottom of the Rhombus from middle to the line before "1" ( the base case of the recursion )
    {                                // it enters only if n ( number chosen by the user ) is greater than one 
